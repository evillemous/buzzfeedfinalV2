...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/debug-login', async (req, res) => {
    try {
      ...
      if (!admin) {
        // Import auth utilities so we can hash passwords
        ...
        console.log('Created new admin user for emergency login');
        
        // Set session manually
        req.session.userId = newAdmin.id;
        // Store additional data in a safe way (TypeScript interface doesn't matter at runtime)
        (req.session as any).adminEmergencyAccess = true;
      ...
      // Set session manually with multiple props to help debugging
      req.session.userId = admin.id;
      // Store additional data in a safe way (TypeScript interface doesn't matter at runtime)
      (req.session as any).adminEmergencyAccess = true;
      (req.session as any).loginTime = new Date().toISOString();
      
      // Force session save with callback pattern
      req.session.save((err) => {
        if (err) {
          console.error('Session save error:', err);
          return res.status(500).json({ error: 'Session save failed', details: err.message });
  ...
  // Emergency admin access that doesn't rely on sessions
  app.get('/api/emergency-admin', async (req, res) => {
    try {
      ...
      // Return admin user data directly without using sessions
      const { password: _, ...userWithoutPassword } = admin;
      ...
      res.status(500).json({ 
        error: 'Emergency admin access failed', 
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });
